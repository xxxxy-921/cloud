...
func TestPlanService_CreatePlan(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-plan", "")
	if err != nil {
...
func TestPlanService_SetDefaultPlan(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-default", "")
	if err != nil {
		t.Fatalf("setup failed: %v", err)
	}

	plan1, err := planSvc.CreatePlan(product.ID, "Plan 1", nil, 0)
	if err != nil {
		t.Fatalf("setup failed: %v", err)
	}
	plan2, err := planSvc.CreatePlan(product.ID, "Plan 2", nil, 0)
	if err != nil {
		t.Fatalf("setup failed: %v", err)
	}
	...
	var count int64
	db.Model(&Plan{}).Where("product_id = ? AND is_default = ?", product.ID, true).Count(&count)
	if count != 1 {
...
func TestPlanService_UpdatePlan(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-update", "")
	if err != nil {
		t.Fatalf("setup failed: %v", err)
	}
