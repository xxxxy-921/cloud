...
func newLicenseService(db *database.DB) *LicenseService {
	return &LicenseService{
		...
		licenseeRepo:     &LicenseeRepo{db: db},
		keyRepo:          &ProductKeyRepo{db: db},
		regRepo:          &LicenseRegistrationRepo{db: db},
		db:               db,
...
func TestLicenseService_IssueLicense_HappyPath(t *testing.T) {
	...
	licenseeSvc := &LicenseeService{repo: &LicenseeRepo{db: db}}
	licenseSvc := newLicenseService(db)

	product, err := productSvc.CreateProduct("Product", "prod-issue", "")
	if err != nil {
		t.Fatalf("setup product failed: %v", err)
	}
	if err := productSvc.UpdateStatus(product.ID, StatusPublished); err != nil {
		t.Fatalf("publish product failed: %v", err)
	}

	licensee, err := licenseeSvc.CreateLicensee(CreateLicenseeParams{Name: "Acme Corp", Notes: ""})
	if err != nil {
	...
	if license.LifecycleStatus != LicenseLifecycleActive {
		t.Errorf("LifecycleStatus = %q, want %q", license.LifecycleStatus, LicenseLifecycleActive)
	}
	...
	var boundReg LicenseRegistration
...
func TestLicenseService_IssueLicense_Guards(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-guard", "")
	if err != nil {
		t.Fatalf("setup product failed: %v", err)
	}
	...
	t.Run("inactive licensee", func(t *testing.T) {
		if err := licenseeSvc.UpdateLicenseeStatus(licensee.ID, LicenseeStatusArchived); err != nil {
		...
		if err := licenseeSvc.UpdateLicenseeStatus(licensee.ID, LicenseeStatusActive); err != nil {
...
func TestLicenseService_UpgradeLicense(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-upgrade", "")
	if err != nil {
		t.Fatalf("setup product failed: %v", err)
	}
	if err := productSvc.UpdateStatus(product.ID, StatusPublished); err != nil {
	...
	if orig.LifecycleStatus != LicenseLifecycleRevoked {
		t.Errorf("original lifecycle = %q, want %q", orig.LifecycleStatus, LicenseLifecycleRevoked)
	}
	if orig.Status != LicenseStatusRevoked {
		t.Errorf("original status = %q, want %q", orig.Status, LicenseStatusRevoked)
	}
...
func TestLicenseService_LifecycleStateMachine(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-lifecycle", "")
	if err != nil {
		t.Fatalf("setup product failed: %v", err)
	}
	if err := productSvc.UpdateStatus(product.ID, StatusPublished); err != nil {
	...
	issue := func(code string) *License {
	...
	t.Run("revoke sets timestamps", func(t *testing.T) {
		...
		if updated.Status != LicenseStatusRevoked {
			t.Errorf("status = %q, want %q", updated.Status, LicenseStatusRevoked)
		}
		if updated.LifecycleStatus != LicenseLifecycleRevoked {
			t.Errorf("lifecycle = %q, want %q", updated.LifecycleStatus, LicenseLifecycleRevoked)
		}
	...
	t.Run("suspend and reactivate", func(t *testing.T) {
		...
		var suspended License
		db.First(&suspended, l.ID)
		if suspended.LifecycleStatus != LicenseLifecycleSuspended {
			t.Errorf("lifecycle = %q, want %q", suspended.LifecycleStatus, LicenseLifecycleSuspended)
		}
		...
		var reactivated License
		db.First(&reactivated, l.ID)
		if reactivated.LifecycleStatus != LicenseLifecycleActive {
			t.Errorf("lifecycle = %q, want %q", reactivated.LifecycleStatus, LicenseLifecycleActive)
		}
	...
	t.Run("renew extends expiration", func(t *testing.T) {
		...
		if renewed.LifecycleStatus != LicenseLifecycleActive {
			t.Errorf("lifecycle = %q, want %q", renewed.LifecycleStatus, LicenseLifecycleActive)
		}
...
func TestLicenseService_BulkReissueLicenses(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-reissue", "")
	if err != nil {
		t.Fatalf("setup product failed: %v", err)
	}
	if err := productSvc.UpdateStatus(product.ID, StatusPublished); err != nil {
...
func TestLicenseService_ExportLicFile(t *testing.T) {
	...
	product, err := productSvc.CreateProduct("Product", "prod-export", "")
	if err != nil {
		t.Fatalf("setup product failed: %v", err)
	}
	if err := productSvc.UpdateStatus(product.ID, StatusPublished); err != nil {
