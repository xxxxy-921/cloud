...
func TestProductService_CreateProduct(t *testing.T) {
	...
	if product.Status != StatusUnpublished {
		t.Errorf("Status = %q, want %q", product.Status, StatusUnpublished)
	}

	// Assert that exactly one ProductKey was created and marked current
...
func TestProductService_UpdateStatus(t *testing.T) {
	...
	product, err := svc.CreateProduct("Product", "prod-1", "")
	if err != nil {
	...
	tests := []struct {
		...
		{"unpublished -> published", StatusUnpublished, StatusPublished, false, nil},
		{"published -> unpublished", StatusPublished, StatusUnpublished, false, nil},
		{"published -> archived", StatusPublished, StatusArchived, false, nil},
		{"archived -> unpublished", StatusArchived, StatusUnpublished, false, nil},
		{"unpublished -> archived", StatusUnpublished, StatusArchived, false, nil},
		{"archived -> published", StatusArchived, StatusPublished, true, ErrInvalidStatusTransition},
		{"published -> published", StatusPublished, StatusPublished, true, ErrInvalidStatusTransition},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Reset product status for each case
			db.Model(&Product{}).Where("id = ?", product.ID).Update("status", tt.from)
...
func TestProductService_RotateKey(t *testing.T) {
	...
	product, err := svc.CreateProduct("Product", "prod-rotate", "")
	if err != nil {
		t.Fatalf("setup failed: %v", err)
	}
...
func TestProductService_CreateProduct_DuplicateCode(t *testing.T) {
	...
	_, err := svc.CreateProduct("Product A", "same-code", "")
	if err != nil {
		t.Fatalf("first create failed: %v", err)
	}

	_, err = svc.CreateProduct("Product B", "same-code", "")
	if err == nil {
