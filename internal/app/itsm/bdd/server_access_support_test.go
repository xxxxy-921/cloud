...
// serverAccessCasePayloads provides routing fixtures for the core, boundary,
// synonym, and default-security server access cases.
var serverAccessCasePayloads = map[string]serverAccessCasePayload{
	"ops": {
		...
		FormData: map[string]any{
			"request_kind":      "application_diagnosis",
	...
	"ops_synonym": {
		Summary:     "生产服务器临时访问申请：需要登录生产机做应用诊断与日志查看。",
		OpenMessage: "我需要临时登录生产应用主机做应用诊断，顺便查看运行日志。",
		FormData: map[string]any{
			"request_kind":      "application_diagnosis",
			"target_host":       "prod-app-05",
			"target_servers":    "prod-app-05",
			"access_account":    "ops.reader",
			"source_ip":         "10.20.30.45",
			"access_window":     "今晚 20:30 到 21:30",
			"operation_purpose": "登录生产应用主机做应用诊断并查看运行日志。",
			"access_reason":     "需要查看应用日志和运行状态，定位线上异常。",
		},
		ExpectedPosition: "ops_admin",
	},
	"ops_reordered": {
		Summary:     "生产服务器临时访问申请：需要查看应用日志并检查进程状态。",
		OpenMessage: "我先看日志再检查进程状态，定位生产服务异常。",
		FormData: map[string]any{
			"request_kind":      "ops_troubleshooting",
			"target_host":       "prod-app-06",
			"target_servers":    "prod-app-06",
			"access_account":    "ops.reader",
			"source_ip":         "10.20.30.46",
			"access_window":     "今晚 21:00 到 22:00",
			"operation_purpose": "查看应用日志并检查进程状态。",
			"access_reason":     "日志排查和进程异常定位属于生产运维排障。",
		},
		ExpectedPosition: "ops_admin",
	},
	"ops_long_sentence": {
		Summary:     "生产服务器临时访问申请：需要对生产应用做长链路运维排查。",
		OpenMessage: "线上服务间歇异常，我要进生产机做应用进程排障，查看应用日志和运行状态，并补充一次主机巡检。",
		FormData: map[string]any{
			"request_kind":      "host_inspection",
			"target_host":       "prod-app-07",
			"target_servers":    "prod-app-07",
			"access_account":    "ops.reader",
			"source_ip":         "10.20.30.47",
			"access_window":     "今晚 22:00 到 23:00",
			"operation_purpose": "查看应用日志和运行状态，并做主机巡检。",
			"access_reason":     "需要做应用进程排障、查看应用日志和运行状态，属于一般生产运维。",
		},
		ExpectedPosition: "ops_admin",
	},
	"network": {
	...
	"network_combined": {
		Summary:     "生产服务器临时访问申请：需要做网络链路与策略联合诊断。",
		OpenMessage: "我要登录网关主机抓包核对链路连通性，并检查 ACL 和防火墙策略。",
		FormData: map[string]any{
			"request_kind":      "network_diagnostic",
			"target_host":       "prod-gateway-02",
			"target_servers":    "prod-gateway-02",
			"access_account":    "net.audit",
			"source_ip":         "10.20.30.48",
			"access_window":     "今晚 22:30 到 23:30",
			"operation_purpose": "抓包核对链路连通性。",
			"access_reason":     "抓包核对链路连通性和防火墙策略，并检查 ACL 调整是否生效。",
		},
		ExpectedPosition: "network_admin",
	},
	"security": {
		...
		FormData: map[string]any{
			"request_kind":      "security_investigation",
	...
	"security_combined": {
		Summary:     "生产服务器临时访问申请：需要做安全审计与异常访问核查。",
		OpenMessage: "安全这边要登录生产机做安全审计，同时核查异常访问痕迹并保全证据。",
		FormData: map[string]any{
			"request_kind":      "security_investigation",
			"target_host":       "prod-app-08",
			"target_servers":    "prod-app-08",
			"access_account":    "sec.audit",
			"source_ip":         "10.20.30.49",
			"access_window":     "今晚 23:00 到 23:50",
			"operation_purpose": "登录生产主机做安全审计，并核查异常访问痕迹。",
			"access_reason":     "需要做安全审计、异常访问核查和证据保全。",
		},
		ExpectedPosition: "security_admin",
	},
	"boundary_security": {
	...
	"default_security": {
		Summary:     "生产服务器临时访问申请：未分类事项默认走安全审核。",
		OpenMessage: "我要进入生产环境核对一个高敏访问事项，但现在还无法明确归类到运维还是网络。",
		FormData: map[string]any{
			"target_host":       "prod-app-09",
			"target_servers":    "prod-app-09",
			"access_account":    "sec.audit",
			"source_ip":         "10.20.30.50",
			"access_window":     "今晚 19:00 到 20:00",
			"operation_purpose": "核对一个高敏访问事项。",
			"access_reason":     "需要核对高敏访问和安全审计事项，暂不涉及运维排障或网络策略变更。",
		},
		ExpectedPosition: "security_admin",
	},
}
