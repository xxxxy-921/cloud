Feature: 数据库备份白名单临时放行 — 智能引擎 Action 元调用
  智能引擎通过 decision.execute_action 工具在 ReAct 循环内同步执行预检和放行动作，
  验证 Action 元调用模型下的"预检→DBA审批→放行→完成"全流程。
