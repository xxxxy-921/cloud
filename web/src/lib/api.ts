...
async function request<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (res.status === 409) {
    ...
    window.dispatchEvent(new CustomEvent("password-expired", { detail: { message } }))
...
export const taskApi = {
  ...
  get: (name: string) =>
    api.get<{ task: TaskInfo; recentExecutions: TaskExecution[] }>(`/api/v1/tasks/${name}`),

  executions: (name: string, page = 1, pageSize = 20) =>
    api.get<{ list: TaskExecution[]; total: number; page: number; pageSize: number }>(
