...
系统 SHALL 在 `cmd/server/` 下提供 edition 文件来控制可选 App 的编译。默认的 `edition_full.go`（带 `//go:build !edition_lite` 约束）SHALL blank-import 所有可选 App 包。`edition_lite.go`（带 `//go:build edition_lite` 约束）SHALL 为空文件，仅编译内核。
...
- **THEN** `edition_lite.go` SHALL 生效，可选 App 的代码 SHALL 不被编译，二进制中不包含这些代码

#### Scenario: 新增 edition 变体
- **WHEN** 需要新的版本组合（如 system+ai）
- **THEN** 开发者 SHALL 创建新的 `edition_<name>.go` 文件，带对应 build tag，blank-import 所需的 App 子集
