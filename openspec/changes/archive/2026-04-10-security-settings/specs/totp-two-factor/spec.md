...
- **THEN** the system SHALL return 400 with message "两步验证已启用"
...
- **THEN** the system SHALL return 400 with message "验证码无效"
...
- **THEN** the system SHALL return 401 with message "验证码无效"

#### Scenario: Expired twoFactorToken
- **WHEN** POST /api/v1/auth/2fa/login with an expired twoFactorToken (>5 minutes)
- **THEN** the system SHALL return 401 with message "验证已过期，请重新登录"
