...
**Reason**: 被 `itsm.service_match` 替代。新工具支持自然语言匹配、置信度评分和候选确认流程，而非简单关键词搜索。
**Migration**: Agent 使用 `itsm.service_match` 替代，输入参数从 `keyword` 改为 `query`。

### Requirement: itsm.query_ticket 工具
**Reason**: 服务台智能体的核心职责是提单引导，不需要通用的工单查询。查询能力由 `itsm.my_tickets` 覆盖（用户自己的工单）。
**Migration**: 如需跨用户查询工单，后续在处理协助智能体中恢复此工具。

### Requirement: itsm.cancel_ticket 工具
**Reason**: 被 `itsm.ticket_withdraw` 替代。语义从"取消"改为"撤回"（仅限未处理工单由提交人撤回），更贴合 ITSM 流程。
**Migration**: Agent 使用 `itsm.ticket_withdraw`，输入参数从 `ticket_id` 改为 `ticket_code`。

### Requirement: itsm.add_comment 工具
**Reason**: 服务台智能体不需要添加评论功能。评论是处理人的操作，不属于提单流程。
**Migration**: 后续在处理协助智能体中恢复此工具。

### Requirement: 处理协助 Agent 预置定义
**Reason**: 本次聚焦服务台和决策两个智能体的工具链完善。处理协助智能体后续单独实现。
**Migration**: 无需迁移，后续作为独立 change 实现。
