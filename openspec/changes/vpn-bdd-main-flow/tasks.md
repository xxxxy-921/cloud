...
- [ ] 4.1 修改 `bdd_test.go`：`initializeScenario` 调用 `registerCommonSteps(sc, bc)` + `registerClassicSteps(sc, bc)` + `registerSmartSteps(sc, bc)`
- [ ] 4.2 删除 `features/example.feature` 中的 @wip 模板场景（如果不再需要）
- [ ] 4.3 运行 `go test ./internal/app/itsm/ -run TestBDD -v` 验证 7 个 scenarios 全部 green
