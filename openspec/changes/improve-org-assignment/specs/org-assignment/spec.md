...
### Requirement: Batch update user positions
**Reason**: Replaced by incremental add/remove API to eliminate concurrent race conditions. The batch `PUT /api/v1/org/users/:id/positions` endpoint that replaces all assignments is removed.
**Migration**: Use `POST /api/v1/org/users/:id/positions` to add and `DELETE /api/v1/org/users/:id/positions/:assignmentId` to remove individual assignments.
