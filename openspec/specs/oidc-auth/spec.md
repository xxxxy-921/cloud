...
The App SHALL provide `POST /api/v1/auth/sso/callback` accepting `{code, state}` that completes the OIDC flow and performs JIT user provisioning.
...
- **THEN** SHALL exchange code for tokens, validate ID token via JWKS, extract claims, JIT-create a local User with the source's default role, and return a TokenPair
...
- **THEN** SHALL link the identity to the existing user and return a TokenPair
...
- **THEN** SHALL find existing user via UserConnection, update attributes if changed, return TokenPair
