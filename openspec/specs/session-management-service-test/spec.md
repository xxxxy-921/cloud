# Capability: session-management-service-test

## Purpose
Defines service-layer test requirements and scenarios for the session management module.

## Requirements

### Requirement: Session service test infrastructure
The system SHALL provide a test harness for `SessionService` using an in-memory SQLite database, real GORM repositories, and a real `TokenBlacklist`, consistent with other kernel service tests.

#### Scenario: Setup test database
- **WHEN** a session service test initializes
- **THEN** it SHALL migrate `User` and `RefreshToken` tables into a shared-memory SQLite database

#### Scenario: Setup DI container
- **WHEN** a session service test needs dependencies
- **THEN** it SHALL use `samber/do` to provide the database, `RefreshTokenRepo`, `TokenBlacklist`, and `SessionService`
